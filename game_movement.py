...
def get_player_coordinates(player, connection):

    sql = f'SELECT latitude_deg, longitude_deg FROM airport WHERE ident = \'{player.location}\''
    cursor = connection.cursor()
    cursor.execute(sql)
...
def player_movement(connection, player):
    airport_list = calculate_all_airport_distance(get_all_airport_coordinates(connection), get_player_coordinates(player, connection))
...
def clue_distance_to_musk(connection, player, musk):
    ...
    musk = get_player_coordinates(musk.location, connection)
    player = get_player_coordinates(player.location, connection)
